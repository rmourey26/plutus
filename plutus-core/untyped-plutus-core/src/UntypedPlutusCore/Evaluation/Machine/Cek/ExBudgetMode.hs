...
restricting' slippageFraction (ExRestrictingBudget b@(ExBudget cpuInit memInit)) = ExBudgetMode $ do
    ...
    let spend _ (ExBudget cpuToSpend memToSpend) = do
            ...
            when (cpuLeft' < 0 || memLeft' < 0) $
                throwingWithCauseExc @(CekEvaluationException uni fun) _EvaluationError
                    ...
                    Nothing
    pure $ ExBudgetInfo (CekBudgetSpender spend) (fractionBudget slippageFraction b) $ do
